<template>
  <div class="home qui-fx" id="show" :style="{height:height+'px'}">
    <a-modal title="设备列表" v-model="setTag" @ok="setTag=false" width="1280px" :footer="null">
    ...
    <div class="left" :style="{height:(height-170)+'px'}">
      ...
      <div class="top">
        ...
        <div class="state set qui-fx-jsb qui-fx-jc">
          ...
          <a-button type="primary" style="margin:5px 10px 0 0" @click="set">设备列表</a-button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    addRTSP(name, token, url, user, password) {
      ...
      var url = `http://${this.$store.state.face.root}/api/v1/AddSrcRTSP`;
      $.ajax({
        type: "get",
        url: url,
        data: req,
    ...
    loadtest() {
      ...
      var url = `http://${this.$store.state.face.root}/api/v1/GetSrcWithoutDevice`;
      $.ajax({
        type: "get",
        url: url,
        data: req,
    ...
    delEq(token) {
      ...
      var url = `http://${this.$store.state.face.root}/api/v1/DelSrc`;
      $.ajax({
        type: "get",
        url: url,
        data: req,
    ...
    initSocket() {
      ...
      if (typeof WebSocket === "undefined") {
        ...
        let socket = new WebSocket("ws://192.168.1.28:3004/");
        socket.onopen = () => {
        ...
        socket.onmessage = msg => {
          ...
          if (obj.snapImgBase64) {
            ...
            if (this.contrastList.length > 10) {
              this.contrastList.pop();
            }
      ...
      $self
        ...
        <li ${
          this.photoList[0].type < 0.74
            ? 'class="err"'
            : ''
        } style="height:${(this.height - 180) * 0.2 - 20}px;width:${(this
            .width *
            0.7 -
            48) /
            8 -
            20}px">
        <img src="${this.photoList[0].photo}" alt="">
        ${
          this.photoList[0].type < 0.74
